...
#' Plot a red Kaplan Meier curve in log scale
#' 
...
#'
#' @usage km_log2(time, censor, plotcens)
#' @return
#' A red Kaplan Meier curve in log scale
#' 
#' @export
#'
#' @examples
km_log2 <- function(time, censor, plotcens){
  ...
  plot(xpart, logypart, type = "s",xlab="Years", ylab="Log of survival probability",lty = 2, col = "red")
  ...
  if (plotcens == 1){
    for (i in 1:length(censor))
    {
      ...
      if (tcen != 0)
      {
        points(tcen, logypart[k],cex=.8,pch=1,col="red")
      }
    }
  }
